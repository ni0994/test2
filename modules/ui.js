...
export function setupSearchEvents(callbacks) {
  ...
  input.addEventListener('input', () => {
    if (input.value === '') callbacks.onSearch('');
  });
}
...
export function setupFilterEvents(callbacks) {
  ...
  if (domainSel) {
    domainSel.addEventListener('change', () => callbacks.onFilter(getQuery()));
  }
  if (poolSel) {
    poolSel.addEventListener('change', () => callbacks.onFilter(getQuery()));
  }
  if (difficultySel) {
    difficultySel.addEventListener('change', () => callbacks.onFilter(getQuery()));
  }
  if (shuffleToggle) {
    shuffleToggle.addEventListener('change', () => callbacks.onRender());
  }
  ...
  document
    .getElementById('btn-clear-tag')
    ?.addEventListener('click', () => callbacks.onClearTag());

  document
    .getElementById('btn-clear-bookmarks')
...
export function setupKeyboardAnswerEvents() {
  document.addEventListener('keydown', e => {
    ...
    const cards = [
      ...document.querySelectorAll('.question-card.state-unanswered'),
    ];
...
export function setupHiddenAdmin(callbacks) {
  ...
  document
    ...
    ?.addEventListener('click', async () => {
      if (!confirm('Service Worker キャッシュを削除します。よろしいですか？')) {
        return;
      }
      if ('caches' in window) {
  ...
  document
    .getElementById('btn-reset-answers')
    ?.addEventListener('click', () => {
      if (
        !confirm(
          '全ての回答履歴を消去します。この操作は元に戻せません。よろしいですか？',
        )
      ) {
        return;
      }
  ...
  document
    .getElementById('btn-reset-answers-only')
    ?.addEventListener('click', () => {
      if (
        !confirm(
          '回答履歴のみを消去します。ブックマークは保持されます。この操作は元に戻せません。よろしいですか？',
        )
      ) {
        return;
      }
...
export function setupDomainStatsBlocks(meta) {
  ...
  const domains =
    meta && Array.isArray(meta.domains) ? meta.domains : [];
...
export function setupDashboardToggle() {
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    // isOpen=true（開いている）→ hidden を付けて閉じる
    // isOpen=false（閉じている）→ hidden を外して開く
