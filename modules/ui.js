...
export function setupFilterEvents(callbacks) {
  ...
  document.getElementById('btn-clear-tag')?.addEventListener('click', () => callbacks.onClearTag());
...
export function setupHiddenAdmin(callbacks) {
  ...
  document.getElementById('btn-reset-answers-only')?.addEventListener('click', () => {
  ...
  document.getElementById('btn-admin-close')?.addEventListener('click', () => panel.classList.add('hidden'));
}
...
export function setupDomainStatsBlocks(meta) {
  ...
  domains.forEach(d => {
    ...
    el.appendChild(label); el.appendChild(body); el.appendChild(bar);
    wrap.appendChild(el);
  });
}
